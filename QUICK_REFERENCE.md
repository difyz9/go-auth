...
cryptoConfig.ForceEncryption = false   // 不强制加密
cryptoConfig.SkipPaths = []string{"/health"} // 跳过路径
```
...
// 重要：注意中间件顺序！
r.Use(cryptoMiddleware.DecryptRequest())   // 1. 解密请求
r.Use(goauth.New(config).Authenticate())    // 2. 认证
r.Use(cryptoMiddleware.EncryptResponse())   // 3. 加密响应
```
...
encrypted, _ := goauth.AESEncrypt(aesKey, jsonData)
...
```go
// 加密
encrypted, err := goauth.AESEncrypt(key, plaintext)

// 解密
decrypted, err := goauth.AESDecrypt(key, encrypted)
```
...
```go
// 认证 + 加解密
authConfig := goauth.QuickConfig("app", "secret")
cryptoConfig := goauth.NewCryptoConfig("aes-key-32-bytes")
